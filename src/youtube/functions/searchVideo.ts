...
export async function searchVideo(query: string): Promise<Video[]> {
  ...
  const videos: any[] =
    response.data.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer
    ...
    .map((data: any) => {
      ...
      return {
        ...
        duration: data.videoRenderer.lengthText
          ? parseDuration(data.videoRenderer.lengthText.simpleText)
          : 0,
        thumbnail: {
          id: data.videoRenderer.videoId,
        ...
        channel: {
          id:
            data.videoRenderer.ownerText.runs[0].navigationEndpoint.browseEndpoint.browseId || null,
          name: data.videoRenderer.ownerText.runs[0].text || null,
          icon: {
            url:
              data.videoRenderer.channelThumbnail?.thumbnails?.[0]?.url ||
              data.videoRenderer.channelThumbnailSupportedRenderers
                ?.channelThumbnailWithLinkRenderer?.thumbnail?.thumbnails?.[0]?.url,
            width:
              data.videoRenderer.channelThumbnail?.thumbnails?.[0]?.width ||
              data.videoRenderer.channelThumbnailSupportedRenderers
                ?.channelThumbnailWithLinkRenderer?.thumbnail?.thumbnails?.[0]?.width,
            height:
              data.videoRenderer.channelThumbnail?.thumbnails?.[0]?.height ||
              data.videoRenderer.channelThumbnailSupportedRenderers
                ?.channelThumbnailWithLinkRenderer?.thumbnail?.thumbnails?.[0]?.height,
          },
          verified: Boolean(
            (
              data.videoRenderer.ownerBadges && data.videoRenderer.ownerBadges[0]
